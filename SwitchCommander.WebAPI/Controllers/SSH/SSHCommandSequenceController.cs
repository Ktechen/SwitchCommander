...
public class SSHCommandSequenceController : BaseController
{
    ...
    private readonly IValidator<CreateSSHCommandSequenceRequest> _sequenceCommandValidator;
    private readonly ILogger<SSHCommandSequenceController> _logger;

    public SSHCommandSequenceController(
        IMediator mediator, 
        IValidator<CreateSSHSequenceRequest> sequenceValidator,
        ILogger<SSHCommandSequenceController> logger, 
        IValidator<CreateSSHCommandSequenceRequest> sequenceCommandValidator) : base(mediator)
    {
        _sequenceValidator = sequenceValidator;
        _logger = logger;
        _sequenceCommandValidator = sequenceCommandValidator;
    }
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult> Create([FromBody] CreateSSHCommandSequenceRequest request,
        CancellationToken cancellationToken)
    {
        if (request is null)
        {
        ...
        var validationResult = await _sequenceCommandValidator.ValidateAsync(request, cancellationToken);
